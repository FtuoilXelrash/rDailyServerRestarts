...
1. **Daily Check** - Every frame, the plugin checks if it's time for the scheduled restart
2. **Countdown Sequence** - When restart time arrives, the plugin announces countdowns at configured intervals
...
The server will automatically restart when the quit command completes (typically handled by an external process manager or systemd).
